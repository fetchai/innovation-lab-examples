...
async def handle_prompt_request(ctx: Context, sender: str, msg: PromptRequest):
    ...
    if not msg.prompt or not msg.prompt.strip():
        ...
        await ctx.send(sender, PromptResponse(response="", error="Prompt cannot be empty."))
        return
    ...
    if client is None:
        ...
        await ctx.send(
            sender, 
            PromptResponse(response="", error="Gemini client not initialized. Check API key.")
        )
        return
    ...
    try:
        ...
        if response.text:
            ...
            await ctx.send(sender, PromptResponse(response="", error="Gemini returned an empty response."))

    # 3. Error handling: API failures or network failures
    except Exception as e:
        ctx.logger.error(f"Error querying Gemini API: {str(e)}")
        await ctx.send(sender, PromptResponse(response="", error=f"Gemini API Error: {str(e)}"))
