...
def create_resource_chat(asset_id: str, uri: str, caption: str = None) -> ChatMessage:
    ...
    content = [
        ResourceContent(
            ...
            resource=Resource(
                uri=uri, metadata={"mime_type": "image/png", "role": "generated-image"}
            ),
        )
    ]
...
async def startup(ctx: Context):
    ...
    if agentverse_api_key:
        ...
        ctx.logger.warning(
            "⚠️  Agentverse API key not set - images won't display in ASI One"
        )
...
Just describe what you want to see!"""
        ...
        await ctx.send(
            sender,
            create_text_chat("🎨 Generating your image... This may take a moment."),
        )
        ...
        response = client.models.generate_content(
            model=IMAGE_MODEL, contents=[user_prompt]
        )
        ...
        if not image_parts:
            error_msg = (
                "❌ Sorry, I couldn't generate an image. Please try a different prompt."
            )
