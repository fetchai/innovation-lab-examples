...
from enum import Enum as PythonEnum  # noqa: E402

from notte_sdk import NotteClient, RemoteSession, retry  # noqa: E402
...
class ProductCategory(BaseModel):
    ...
    menu: Optional[str] = Field(
        None,
        description="The menu group of the product category (e.g., 'Shoes', 'Clothing')",
    )
    parent_category: Optional[str] = Field(
        None, description="The parent category if this is a sub-category"
    )
    filters_applied: Optional[Dict[str, Any]] = Field(
        None,
        description="Any filters applied to get this category list (e.g., color, size)",
    )
...
class ProductCategories(BaseModel):
    ...
    source_url: str = Field(
        description="The URL from which these categories were scraped"
    )
        ...
        return ProductCategories(
            categories=[
                ProductCategory(
                    name="Running Shoes",
                    menu="Shoes",
                    url=f"{NIKE_BASE_URL}/w/mens-running-shoes-37v7jzy7ok",
                ),
                ProductCategory(
                    name="Lifestyle Sneakers",
                    menu="Shoes",
                    url=f"{NIKE_BASE_URL}/w/mens-lifestyle-shoes-13jrmznik1",
                ),
                ProductCategory(
                    name="Basketball Shorts",
                    menu="Clothing",
                    parent_category="Shorts",
                    url=f"{NIKE_BASE_URL}/w/mens-basketball-shorts-38fphz98uk",
                ),
            ],
...
class ShoppingItem(BaseModel):
    ...
    price: Optional[float] = Field(
        None,
        description="The price of the product. Can be null if price varies or is not listed.",
    )
    original_price: Optional[float] = Field(
        None, description="The original price if the item is on sale"
    )
    ...
    image_src: Optional[str] = Field(
        None, description="The URL or src path to the product image"
    )
    category: Optional[str] = Field(
        None, description="The category the product belongs to"
    )
    product_id: Optional[str] = Field(
        None, description="Unique product identifier if available"
    )
    available_sizes: Optional[List[str]] = Field(
        None, description="List of available sizes"
    )
    color: Optional[str] = Field(None, description="Primary color of the product shown")
    rating: Optional[float] = Field(
        None, description="Average customer rating (e.g., 4.5)"
    )
...
class ShoppingList(BaseModel):
    ...
    category_scraped: Optional[str] = Field(
        None, description="The specific category these items belong to"
    )
    total_items_found: Optional[int] = Field(
        None, description="Total items found for the query, even if not all are listed"
    )
    current_page: Optional[int] = Field(
        1, description="Current page number if paginated"
    )
    total_pages: Optional[int] = Field(
        None, description="Total pages available if paginated"
    )
...
class NikeAction(str, PythonEnum):
    ...
    GET_SUB_CATEGORIES = (
        "get_sub_categories"  # e.g., Men's Running Shoes under Men's Shoes
    )
    # Product focused
...
def _build_nike_url(
    ...
    category_path: Optional[
        str
    ] = None,  # e.g., "w/mens-running-shoes-37v7jzy7ok" or "w/new-3n82y"
...
async def _scrape_nike_data_with_notte(
    ...
    try:
        ...
        response = (
            NotteClient(api_key=NOTTE_API_KEY)
            .Agent(session=session, max_steps=max_steps)
            .run(
            ...
            try:
                ...
                if match:
                    ...
                    try:
                        ...
                        print(
                            f"Error validating JSON from markdown for {url}: {val_err_md}. JSON string: {json_str[:200]}..."
                        )
                        return None
                else:
                    print(
                        f"Error validating direct JSON for {url}: {val_err_direct}. No markdown JSON found. Answer: {response.answer[:200]}..."
                    )
                    return None
        else:
            print(
                f"No answer received from Notte agent for URL: {url}. Error: {response.error if hasattr(response, 'error') else 'Unknown'}"
            )
            return None
...
@retry(max_tries=3)
async def scrape_main_categories(
    session: RemoteSession, gender: GenderTarget = GenderTarget.MEN
...
"""
    return await _scrape_nike_data_with_notte(
        target_url, task, ProductCategories, session
    )
...
async def scrape_products_by_category(
    ...
    if sort_by:
        sort_instructions = (
            f"Sort the products by {sort_by.name.lower().replace('_', ' ')}. "
        )
...
async def search_nike_products(
    ...
    target_url = _build_nike_url(
        search_query=search_term, gender=gender
    )  # Search URL might not always include gender directly

    filter_instructions = ""
    if filters:
        filter_str = ", ".join([f"{k} as {v}" for k, v in filters.items()])
        filter_instructions = (
            f"After searching, if possible, apply filters: {filter_str}. "
        )

    sort_instructions = ""
    if sort_by:
        sort_instructions = (
            f"Sort the search results by {sort_by.name.lower().replace('_', ' ')}. "
        )
...
async def scrape_product_details(
    product_url_or_name: str, session: RemoteSession, is_url: bool
) -> Optional[ShoppingItem]:
    """Fetches detailed information for a single Nike product, either by URL or by searching its name."""
    target_url = (
        product_url_or_name
        if is_url
        else _build_nike_url(search_query=product_url_or_name)
    )

    initial_step = (
        f"Go to the product page: {target_url}."
        if is_url
        else f"Search for the product '{product_url_or_name}' on Nike.com (you can start at {target_url}) and navigate to its main product page."
    )
...
async def scrape_sub_categories(
    ...
    try:
        ...
        if raw_scraped_data and raw_scraped_data.categories:
            ...
            if (
                not sub_categories_list
            ):  # If parsing resulted in an empty list but raw_data had categories
                print(
                    f"Sub-category parsing resulted in empty list for {main_category_name} at {main_category_url}, though Notte might have returned data."
                )
                return None
            return ProductCategories(
                categories=sub_categories_list, source_url=main_category_url
            )
        else:
            print(
                f"No sub-categories found or failed to parse for {main_category_name} at {main_category_url}"
            )
            return None
    except Exception as e:
        print(
            f"Error scraping sub-categories for {main_category_name} from {main_category_url}: {e}"
        )
        return None
...
async def get_nike_info_enhanced(request: NikeScrapeRequest) -> NikeScrapeResponse:
    ...
    if not NOTTE_API_KEY:
        return NikeScrapeResponse(
            results="Error: NOTTE_API_KEY is not set. Please configure it."
        )
    ...
    try:
        ...
        with notte.Session(headless=False) as session:
            ...
            max_items_to_return = (
                request.max_items if request.max_items and request.max_items > 0 else 5
            )

            if (
                action == NikeAction.GET_MAIN_CATEGORIES
                or action == NikeAction.LIST_CATEGORIES
            ):
                categories_data = await scrape_main_categories(
                    session=session, gender=request.gender
                )
                if categories_data and categories_data.categories:
                    ...
                    response_str += "\n".join(
                        [
                            f"- {cat.name} ({cat.menu if cat.menu else 'General'}) - URL: {cat.url}"
                            for cat in categories_data.categories[:max_items_to_return]
                        ]
                    )
                ...
                action == NikeAction.LIST_PRODUCTS_BY_CATEGORY
                or action == NikeAction.LIST_PRODUCTS
            ):
                category_to_process = (
                    request.category_name.strip() if request.category_name else None
                )
                actual_category_name_found = (
                    None  # To store the name of the category as found on Nike's site
                )
                ...
                if category_to_process and request.gender:
                    ...
                    for prefix in prefixes_to_remove:
                        if category_to_process.lower().startswith(prefix):
                            category_to_process = category_to_process[
                                len(prefix) :
                            ].strip()
                            print(
                                f"Cleaned category_to_process to: '{category_to_process}' after removing gender prefix '{prefix.strip()}'"
                            )
                            break
                ...
                if (
                    action == NikeAction.LIST_PRODUCTS_BY_CATEGORY
                    and not category_to_process
                ):
                    response_str = "Error: Category name is required for listing products by category."
                    return NikeScrapeResponse(
                        results=response_str, raw_data=raw_data_response
                    )

                if (
                    not category_to_process
                ):  # Implies action == NikeAction.LIST_PRODUCTS and no category given
                    print(
                        f"No category specified for LIST_PRODUCTS. Attempting to find a default like 'New Arrivals' for {request.gender.value}."
                    )
                    main_categories_data = await scrape_main_categories(
                        session=session, gender=request.gender
                    )
                    if main_categories_data and main_categories_data.categories:
                        ...
                        for cat_pattern in [
                            ...
                            for cat in main_categories_data.categories:
                                if cat_pattern in cat.name.lower():
                                    ...
                                    print(
                                        f"Found default category: '{actual_category_name_found}' at {target_category_url}"
                                    )
                                    break
                            if target_category_url:
                                break
                            ...
                            for cat_pattern in [
                                ...
                                for cat in main_categories_data.categories:
                                    if cat_pattern in cat.name.lower():
                                        ...
                                        actual_category_name_found = cat.name
                                        print(
                                            f"Found fallback default category: '{actual_category_name_found}' at {target_category_url}"
                                        )
                                        break
                                if target_category_url:
                                    break
                        ...
                        if (
                            not target_category_url and main_categories_data.categories
                        ):  # Still nothing, take the very first one
                            target_category_url = main_categories_data.categories[0].url
                            actual_category_name_found = (
                                main_categories_data.categories[0].name
                            )
                            print(
                                f"Using first main category as default: '{actual_category_name_found}' at {target_category_url}"
                            )
                    ...
                    if main_categories_data and main_categories_data.categories:
                        for cat in main_categories_data.categories:
                            if category_to_process.lower() in cat.name.lower():
                                ...
                                actual_category_name_found = cat.name
                                print(
                                    f"Found '{actual_category_name_found}' as a main category. URL: {target_category_url}"
                                )
                                break

                    if (
                        not target_category_url
                        and main_categories_data
                        and main_categories_data.categories
                    ):
                        print(
                            f"'{category_to_process}' not found as a main category. Checking sub-categories..."
                        )
                        for main_cat in main_categories_data.categories:
                            print(
                                f"Checking sub-categories under '{main_cat.name}' for '{category_to_process}'..."
                            )
                            ...
                            if sub_categories_data and sub_categories_data.categories:
                                for sub_cat in sub_categories_data.categories:
                                    if (
                                        category_to_process.lower()
                                        in sub_cat.name.lower()
                                    ):
                                        target_category_url = sub_cat.url
                                        actual_category_name_found = sub_cat.name
                                        print(
                                            f"Found '{actual_category_name_found}' as a sub-category under '{main_cat.name}'. URL: {target_category_url}"
                                        )
                                        break  # Found the sub-category
                            if (
                                target_category_url
                            ):  # If found in current main_cat's sub-categories
                                break

                if target_category_url and actual_category_name_found:
                    print(
                        f"Proceeding to scrape products from: '{actual_category_name_found}' at {target_category_url}"
                    )
                    ...
                    if products_data and products_data.items:
                        ...
                        for prod in products_data.items:  # Already limited by max_items in scrape_products_by_category
                            ...
                            response_str += (
                                f"- {prod.name} - {price_str} - URL: {prod.url}\n"
                            )
                        if (
                            products_data.total_items_found
                            and products_data.total_items_found
                            > len(products_data.items)
                        ):
                ...
                if not request.search_term:
                    return NikeScrapeResponse(
                        results="Error: Search term is required for searching products."
                    )
                ...
                if search_results and search_results.items:
                    ...
                    for item in search_results.items:
                        ...
                        response_str += (
                            f"- {item.name} ({price_info}) - URL: {item.url}\n"
                        )
                    if (
                        search_results.total_items_found
                        and search_results.total_items_found > len(search_results.items)
                    ):
                ...
                product_to_find = (
                    request.product_url or request.product_name_for_details
                )
                if not product_to_find:
                    return NikeScrapeResponse(
                        results="Error: Product URL or name is required to get details."
                    )

                is_url_provided = bool(request.product_url)
                product_detail = await scrape_product_details(
                    product_to_find, session, is_url=is_url_provided
                )

                if product_detail:
                    response_str = f"Details for {product_detail.name}:\n"
                    price_info = (
                        f"${product_detail.price:.2f} {product_detail.currency}"
                        if product_detail.price is not None
                        else "Price not available"
                    )
                    if (
                        product_detail.original_price
                        and product_detail.price is not None
                        and product_detail.original_price > product_detail.price
                    ):
                        price_info += (
                            f" (Original: ${product_detail.original_price:.2f})"
                        )
                    response_str += f"- Price: {price_info}\n"
                    response_str += f"- URL: {product_detail.url}\n"
                    if product_detail.product_id:
                        response_str += f"- Product ID: {product_detail.product_id}\n"
                    if product_detail.color:
                        response_str += f"- Color: {product_detail.color}\n"
                    if product_detail.available_sizes:
                        response_str += f"- Available Sizes: {', '.join(product_detail.available_sizes)}\n"
                    if product_detail.rating:
                        response_str += f"- Rating: {product_detail.rating}/5 ({product_detail.review_count or 0} reviews)\n"
                    if product_detail.image_src:
                        response_str += f"- Image: {product_detail.image_src}\n"
                    # Could add description if needed, but might be too long for chat
                    raw_data_response = product_detail.model_dump()
                else:
                    response_str = (
                        f"Could not retrieve details for '{product_to_find}'."
                    )
            ...
            # Placeholder for GET_SUB_CATEGORIES, FIND_BESTSELLERS, FIND_NEW_ARRIVALS
                ...
                if not request.category_name:
                    return NikeScrapeResponse(
                        results="Error: Main category name (e.g., 'Shoes') is required to get sub-categories."
                    )

                main_categories_data = await scrape_main_categories(
                    session, request.gender
                )
        ...
        response_str = (
            f"An unexpected error occurred: {str(e)[:100]}... Please check logs."
        )
