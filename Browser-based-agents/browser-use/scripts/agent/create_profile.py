...
parser.add_argument(
    "--agent-address",
    help="Save to Supabase under this agent address instead of the default local CLI key",
)
...
print(
    f"This will save to Supabase (agent_address={args.agent_address or LOCAL_AGENT_ADDRESS})\n"
)
...
p = UserProfile(
    ...
    skills=[
        s.strip()
        for s in prompt("Top skills (comma-separated)").split(",")
        if s.strip()
    ],
    # Auth
    cerebralvalley_email=prompt(
        "Cerebral Valley email (for registration login, optional)"
    ),
