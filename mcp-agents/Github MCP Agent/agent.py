...
class GitHubMCPClient:
    def __init__(self, ctx: Context, access_token: str):
        ...
        self.tools: list = []  # Will be populated after connection
...
async def handle_device_flow_polling(ctx: Context, session_id: str):
    ...
    while True:
        ...
        if result and result != "pending" and result != "slow_down":
            ...
            # get_user_info returns None when GitHub rejects the freshly minted
            # token, which does happen; treat it as a failed login rather than
            # storing a null session and crashing on the log line below.
            if user_info is None:
                ctx.logger.error("Device flow returned a token GitHub would not accept")
                break
            ...
            ctx.logger.info(
                f" User {user_info.get('login', 'Unknown')} authenticated successfully"
            )
            break
