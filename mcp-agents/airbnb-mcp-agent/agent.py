    ...
    async def connect(self):
        ...
        try:
            ...
            self._ctx.logger.info(
                f"Connected to Airbnb MCP server with {len(mcp_tools)} tools"
            )
...
Make sure to provide helpful, accurate information about available properties and booking options.""",
            ...
            if tool_use:
                ...
                self._ctx.logger.info(
                    f"Claude selected Airbnb tool: {tool_name} with input: {tool_input}"
                )
    ...
    def _format_search_results(self, data: Dict) -> str:
        ...
        for i, result in enumerate(search_results[:5], 1):
            ...
            try:
                # Extract name
                if (
                    "demandStayListing" in result
                    and "description" in result["demandStayListing"]
                ):
                    name_obj = result["demandStayListing"]["description"].get(
                        "name", {}
                    )
                ...
                # Extract price
                if (
                    "structuredDisplayPrice" in result
                    and "primaryLine" in result["structuredDisplayPrice"]
                ):
                    price = result["structuredDisplayPrice"]["primaryLine"].get(
                        "accessibilityLabel", ""
                    )
                ...
                if (
                    ...
                    and "primaryLine" in result["structuredContent"]
                ):
        ...
        if len(search_results) > 5:
            formatted_response += (
                f"... and {len(search_results) - 5} more properties available.\n"
            )
    ...
    def _format_airbnb_listings(self, listings: list) -> str:
        ...
        if len(listings) > 5:
            formatted_response += (
                f"... and {len(listings) - 5} more properties available.\n"
            )
...
async def handle_chat_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    try:
        ...
        error_msg = ChatMessage(
            ...
            content=[
                TextContent(
                    type="text", text=f"Sorry, I encountered an error: {str(e)}"
                )
            ],
        )
