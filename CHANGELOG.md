...
- Bare `except:` in `deploy-agent-on-av/example/agent.py` and `frontend-integration/frontend_app.py` swallowed `KeyboardInterrupt` and `SystemExit`, so neither agent could be stopped cleanly mid-request. Narrowed to `Exception` and to `requests.RequestException` respectively. Reported by @saurabhhhcodes in #173
