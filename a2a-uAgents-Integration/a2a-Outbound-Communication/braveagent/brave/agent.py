    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            if message_content.startswith("WEB:"):
                ...
                await self._handle_general_search_command(
                    f"SEARCH:{message_content}", event_queue
                )
        ...
        if not query:
            await event_queue.enqueue_event(
                new_agent_text_message(
                    "❌ Usage: WEB:query (e.g., WEB:Python programming)"
                )
            )
            return
...
✅ Web search by Brave Search Agent
    ...
    async def _handle_local_search_command(self, command: str, event_queue: EventQueue):
        ...
        if not query:
            await event_queue.enqueue_event(
                new_agent_text_message(
                    "❌ Usage: LOCAL:query (e.g., LOCAL:pizza near Central Park)"
                )
            )
            return
...
✅ Local search by Brave Search Agent
    ...
    async def _handle_general_search_command(
        self, command: str, event_queue: EventQueue
    ):
        ...
        if not query:
            await event_queue.enqueue_event(
                new_agent_text_message(
                    "❌ Usage: SEARCH:query (e.g., SEARCH:latest AI news можем"
                )
            )
            return
...
✅ General search by Brave Search Agent
    ...
    async def _handle_summarize_command(self, command: str, event_queue: EventQueue):
        ...
        if not query:
            await event_queue.enqueue_event(
                new_agent_text_message(
                    "❌ Usage: SUMMARIZE:query (e.g., SUMMARIZE:AI advancements 2025)"
                )
            )
            return
...
✅ Summary by Brave Search Agent
    ...
    async def _perform_local_search(self, query: str, count: int = 5) -> str:
        ...
        data = response.json()
        location_ids = [
            r["id"] for r in data.get("locations", {}).get("results", []) if r.get("id")
        ]
        ...
        try:
            poi_response = requests.get(
                self.local_url, params=params, headers=self.headers
            )
            poi_response.raise_for_status()
            desc_response = requests.get(
                self.desc_url, params=params, headers=self.headers
            )
