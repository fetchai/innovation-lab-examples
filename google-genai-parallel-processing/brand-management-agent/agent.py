...
class ASI1MiniProcessor(processor.Processor):
    ...
    async def call(self, content: AsyncIterable[ProcessorPart]):
        ...
        try:
                ...
                response = await client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {
                                "role": "user",
                                "content": f"{self.task} for: {self.prompt}",
                            }
                        ],
                        "temperature": 0.7,
                        "max_tokens": 1024,
            ...
            yield ProcessorPart(
                f"Error during {self.task}: {str(e)}",
                metadata={"type": self.metadata_type, "error": True},
            )
...
class WebLayoutProcessor(ASI1MiniProcessor):
    ...
    def __init__(self, prompt: str):
        super().__init__(
            prompt, "Design a minimal HTML layout for a landing page", "web_layout"
        )
        ...
        try:
            ...
            if response.data and response.data[0].url:
                ...
                if response.status_code == 200:
                    ...
                    yield ProcessorPart(
                        f"Logo uploaded: {self.asset_id}",
                        metadata={"type": "logo_image"},
                    )
                else:
                    yield ProcessorPart(
                        "Failed to download image",
                        metadata={"type": "logo_image", "error": True},
                    )
            else:
                yield ProcessorPart(
            ...
            yield ProcessorPart(
                f"Logo generation failed: {str(e)}",
                metadata={"type": "logo_image", "error": True},
            )
    ...
    for item in msg.content:
        if isinstance(item, TextContent):
            ...
            if logo_processor.asset_id and logo_processor.asset_uri:
                try:
                    external_storage.set_permissions(
                        asset_id=logo_processor.asset_id, agent_address=sender
                    )
...
if __name__ == "__main__":
    ...
    print(
        "💡 Send a message like 'build a around sustainable clothing' to see parallel processing in action!"
    )
    agent.run()
