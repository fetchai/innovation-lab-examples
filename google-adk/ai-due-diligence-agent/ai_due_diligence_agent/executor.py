...
# Initialize uAgent
agent = Agent(name=APP_NAME, seed=AGENT_SEED, port=AGENT_PORT, mailbox=True)
...
async def periodic_cleanup(ctx: Context) -> None:
    ...
    try:
        ...
        for blob in bucket.list_blobs():
            ctx.logger.debug(
                f"Checking artifact: {blob.name}, created at {blob.time_created}"
            )
            if blob.time_created < cutoff:
                ...
                ctx.logger.debug(
                    f"Retained recent artifact: {blob.name}, remaining time: {remaining_time}"
                )
                retained_count += 1

        ctx.logger.info(
            f"Periodic cleanup completed. Deleted: {deleted_count}, Retained: {retained_count}"
        )
...
def initialize_adk_agent():
    ...
    app = App(
        ...
        events_compaction_config=EventsCompactionConfig(
            compaction_interval=30, overlap_size=3
        ),
    )
...
async def _ensure_session_exists(user_id: str, session_id: str):
    ...
    try:
        await runner_session_service.create_session(
            app_name="DueDiligenceAnalyst", user_id=user_id, session_id=session_id
        )
    except Exception:
        try:
...
class StageTracker:
            ...
            if stage:
                ...
                if stage_name not in self.started_stages:
                    ...
                    if stage_name != self.last_notified_start:
                        ...
                        start_notification = (
                            f"{stage_emoji} **Starting {stage_name}...**"
                        )
                        ...
                        if prev_name not in self.completed_stages:
                            ...
                            if prev_name != self.last_notified_complete:
                                ...
                                complete_notification = (
                                    f"{prev_emoji} **{prev_name} Complete**"
                                )
    ...
    def _detect_from_text(self, text: str) -> tuple[str | None, str | None]:
        ...
        explicit_start_markers = {
            ...
            "Investor Memo": [
                "starting investor memo",
                "beginning investor memo",
                "creating investor memo",
            ],
            "Report Generation": [
            ...
            if stage_name not in self.started_stages:
                if any(marker in text_lower for marker in markers):
                    ...
                    if stage_name != self.last_notified_start:
                        self.last_notified_start = stage_name
                        start_notification = (
                            f"{stage['emoji']} **Starting {stage_name}...**"
                        )
                        break
            ...
            if stage_name not in self.completed_stages:
                if any(marker in text_lower for marker in markers):
                    ...
                    if stage_name != self.last_notified_complete:
                        ...
                        complete_notification = (
                            f"{stage['emoji']} **{stage_name} Complete**"
                        )
                        break
...
async def process_with_adk(query: str, sender: str, ctx: Context) -> str:
    ...
    try:
        ...
        async for event in adk_runner.run_async(
            user_id=sender, session_id=str(ctx.session), new_message=new_message
        ):
