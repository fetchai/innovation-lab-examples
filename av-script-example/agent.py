...
agent = Agent(name=UAGENT_NAME, seed=SEED_PHRASE, port=AGENT_PORT, mailbox=True)
    ...
    for item in msg.content:
        if isinstance(item, StartSessionContent):
            ...
            welcome_message = create_text_chat(
                "Hello! I'm an example agent. How can I help you?"
            )
            ...
            goodbye_message = create_text_chat(
                "Goodbye! Thanks for chatting.", end_session=True
            )
...
async def startup_handler(ctx: Context):
    ...
    if AGENTVERSE_KEY and SEED_PHRASE:
        try:
            ...
            ctx.logger.info(
                f"Registering with Agentverse using endpoint: {endpoint_url}"
            )
        ...
        ctx.logger.warning(
            f"⚠️ {', '.join(missing)} not set, skipping Agentverse registration"
        )
