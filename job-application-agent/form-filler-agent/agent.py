...
WELCOME = (
    "👋 I'm the **Job Application Form-Filler**.\n\n"
    "Paste a Greenhouse job link (e.g. "
    "`https://boards.greenhouse.io/<company>/jobs/<id>`) and I'll:\n"
    "  1. Pull the posting + form schema\n"
    "  2. Fill it from your saved profile (with RAG + ASI:One for free-text)\n"
    "  3. Show you every field that will be submitted — you decide when to send.\n\n"
    "**Note:** before your first application, set up your profile via the "
    "Profile Agent (saved resume + structured fields). Type `help` for the "
    "full command list."
)
...
async def _start_application(ctx: Context, sender: str, url: str) -> None:
    ...
    await _say(
        ctx, sender,
        rendering.format_commands_hint(have_missing=bool(sess.missing_required)),
    )
...
async def _handle_review_command(
    ...
    if cmd.kind in {"answer", "edit"}:
        ...
        value = cmd.value or ""
        if not name:
            await _say(ctx, sender, "Usage: `answer <name> <value>`")
            return

        # Validate: name must exist in the form schema.
        if not sess.field_meta(name):
            await _say(
                ctx, sender,
                f"No field named `{name}` in this form. Try `show all` to see "
                f"the field names.",
            )
            return

        # Try to normalise against select option labels → values.
        ...
        sess.user_edits.append(
            {"name": name, "label": label, "value": resolved, "kind": cmd.kind}
        )
        session_mod.save(ctx.storage, sess)

        # Best-effort save-back to the profile.
        await _save_edit_to_profile(
            ctx, label=label, field_name=name, value=resolved
        )

        msg = f"✓ Set `{name}` = `{resolved}` (also saved to your profile)."
        if sess.missing_required:
            msg += f"\nStill missing: {', '.join(sess.missing_required)}"
        else:
            msg += "\n🎉 All required fields are now filled. Type `submit` to send (dry-run on)."
        await _say(ctx, sender, msg)
        return
    ...
    # Unknown command in REVIEWING state.
    await _say(
        ctx, sender,
        "I didn't understand that. Type `help` for the command list.",
    )
