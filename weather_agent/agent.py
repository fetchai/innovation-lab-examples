...
async def handle_weather_request(ctx: Context, sender: str, msg: WeatherRequest):
    ...
    if geo is None:
        await ctx.send(sender, WeatherResponse(
            city=city, temperature_c=0, feels_like_c=0,
            wind_speed_kmh=0, humidity_percent=0, description="",
    ...
    if current is None:
        await ctx.send(sender, WeatherResponse(
            city=display_name, temperature_c=0, feels_like_c=0,
            wind_speed_kmh=0, humidity_percent=0, description="",
    ...
    ctx.logger.info(f"Replying: {display_name} | {response.temperature_c}°C | {response.description}")
