        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "origin": {
                        "type": "string",
                        "description": "IATA code of origin airport (e.g., 'LAX')",
                    },
                    "destination": {
                        "type": "string",
                        "description": "IATA code of destination airport (e.g., 'JFK')",
                    },
                    "date": {
                        "type": "string",
                    ...
                    "passengers": {
                        "type": "integer",
                        "description": "Optional: number of adult passengers (defaults to 1)",
                    },
                    "cabin_class": {
                        "type": "string",
                        "description": "Optional cabin class (e.g., 'economy')",
                    },
                    "preferred_airlines": {
                        "type": "array",
                        "items": {"type": "string"},
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "offer_id": {
                        "type": "string",
                        "description": "Duffel offer ID to refresh",
                    }
                },
                "required": ["offer_id"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "offer_id": {
                        "type": "string",
                        "description": "Offer ID from Duffel",
                    },
                    "passengers": {
                        "type": "array",
                        "items": {"type": "object"},
                    ...
                    "pay_with_balance_now": {
                        "type": "boolean",
                        "description": "If true, ticket immediately by paying now",
                    },
                    "notify_email": {
                        "type": "string",
                        "description": "Optional: email to send booking confirmation",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "currency": {
                        "type": "string",
                        "description": "Currency code of payment",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "notify_email": {
                        "type": "string",
                        "description": "Optional: email to send confirmation",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "order_cancellation_id": {
                        "type": "string",
                        "description": "Cancellation ID from the quote (ocr_...)",
                    }
                },
                "required": ["order_cancellation_id"],
            },
...
{SYSTEM_PROMPT}"""
    ...
    if message.tool_calls:
        ...
        has_wait_message = assistant_content and any(
            phrase in assistant_content.lower() for phrase in wait_phrases
        )

        if has_wait_message:
            # Retry without the wait message - add a system reminder
            logger.info(
                f"Detected 'please wait' message, retrying tool call without it"  # noqa: F541
            )
            history.append(
                {
                    "role": "system",
        ...
        try:
            if func_name == "duffel_search_offers":
                ...
                result = func(
                    slices=[
                        {
                            "origin": func_args["origin"],
                            "destination": func_args["destination"],
                            "departure_date": func_args["date"],
                        }
                    ],
                ...
                if not passengers_arg:
                    ...
                    try:
                        if isinstance(
                            session_state.get("autofill_passenger_data"), dict
                        ):
                        ...
                        if not pax:
                            for h in reversed(history):
                                ...
                                if not isinstance(txt, str):
                                    continue
                                m = re.search(
                                    r"\b(mr|ms|mrs|miss|dr|mx)\.?\b", txt, re.I
                                )
                                if m:
                ...
                if not passengers_arg:
                    result = {
                        "error": "MISSING_PASSENGERS",
                        "message": "Passenger details are required to place booking.",
                    }
                else:
                    ...
                    result = func(
                        ...
                        or (
                            passengers_arg[0].get("email")
                            if isinstance(passengers_arg, list) and passengers_arg
                            else None
                        ),
                    )
                ...
                if not result.get("error"):
                    session_state["pending_cancellation_id"] = result.get(
                        "order_cancellation_id"
                    )
                    session_state["pending_cancellation_order_id"] = func_args[
                        "order_id"
                    ]
                ...
                if not result.get("error"):
                    ...
                    session_state["cancelled_refund_amount"] = result.get(
                        "refund_amount"
                    )
                    session_state["cancelled_refund_currency"] = result.get(
                        "refund_currency"
                    )
                ...
                session_state["payment_description"] = func_args.get(
                    "description", "Flight booking — pay to proceed"
                )
                result = {
        ...
        if func_name == "duffel_get_offer_with_services":
            ...
            if not result.get("error"):
                ...
                if result.get("offer_passengers"):
                    ...
                    logger.info(
                        f"Stored {len(result['offer_passengers'])} passenger IDs from offer"
                    )
